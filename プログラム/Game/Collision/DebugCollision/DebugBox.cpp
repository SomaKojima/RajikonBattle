...
#include "DebugBox.inc"		// モデルの頂点とインデックス情報

#endif

DebugBox::DebugBox(ID3D11Device* device, Vector3 center, Vector3 radius)
{
	SetCenter(center);
	SetRadius(radius);
...
void DebugBox::Draw(ID3D11DeviceContext* context, CommonStates& states, const Matrix& world, const Matrix& view, const Matrix& projection)
{
...
void DebugBox::SetCenter(DirectX::SimpleMath::Vector3 center)
{
	m_center = center;
	m_trans = Matrix::CreateTranslation(m_center);
}

DirectX::SimpleMath::Vector3 DebugBox::GetCenter()
{
	return m_center;
}

void DebugBox::SetRadius(Vector3 radius)
{
	m_radius = radius;
	m_scale = Matrix::CreateScale(m_radius);
}

Vector3 DebugBox::GetRadius()
{
	return m_radius;
}

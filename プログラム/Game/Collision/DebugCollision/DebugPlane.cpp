...
#include "DebugPlane.inc"		// モデルの頂点とインデックス情報

#endif

DebugPlane::DebugPlane(ID3D11Device* device, Vector3 center, DirectX::SimpleMath::Vector3 angle, float width, float height)
{
	SetCenter(center);
...
void DebugPlane::Draw(ID3D11DeviceContext* context, CommonStates& states, const Matrix& world, const Matrix& view, const Matrix& projection)
{
	...
	Matrix WVP = m_scale * m_rotation * m_trans * world * view * projection;
...
void DebugPlane::SetCenter(DirectX::SimpleMath::Vector3 center)
{
	m_center = center;
	m_trans = Matrix::CreateTranslation(m_center);
}

void DebugPlane::SetWidthHeight(float width, float height)
{
	m_width = width;
	m_height = height;
	m_scale = Matrix::CreateScale(Vector3(m_width, m_height, 1));
}

Vector2 DebugPlane::GetWidthHeight()
{
